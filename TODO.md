...
- [x] 伴唱自动检测：分离后检测伴唱 RMS/dBFS，低于阈值（-40dBFS）判定"无伴唱"跳过
- [x] 伴唱转换：使用与主唱相同的模型/引擎转换伴唱（Applio，同一 ConvertConfig）
- [x] 伴唱效果链调整：伴唱混响更湿、更远（区别于主唱的"前方"定位）
  - HPF 80Hz → 轻压缩(-20dB/2.0:1) → 轻EQ(+1/+0.5/+1) → Limiter(-6dB) → Reverb(0.65/0.35)
- [x] 三轨混音集成到 `mix.py`：`mix_tracks()` 支持 `backing_vocals_path` 参数
- [x] `separate.py` 新增 `separate_karaoke()` 函数
- [x] `__main__.py` 集成完整流程：下载 → 分离 → karaoke 分离 → 主唱+伴唱转换 → 三轨混音
- [x] `convert.py` 修复路径 resolve 时序（chdir 前 resolve，防止相对路径错误）
- [x] 新增 `--no-backing` CLI 参数跳过伴唱处理
- [x] 集成测试：60s 片段 + 完整歌曲（255s 伴唱转换 29.7s，三轨混音 2.0s）
- [x] 输出到 iCloud：`v1.3_test/v1.3_newsong_hayley_applio_三轨混音_with_backing.wav`

> **伴唱转换总结**：
> - 伴唱 RMS 检测：-26.4 dBFS（远高于 -40 阈值，确认有伴唱内容）
> - 伴唱转换：255s 全歌 29.7s（8.6:1 实时比，与主唱转换速度相当）
